...
INJECTION_ARTIFACT = Path(tempfile.gettempdir()) / "nullsec_injection_artifact"
...
def test_command_injection() -> bool:
    ...
    payloads = [
        ...
        {"tool": "nmap", "target": "127.0.0.1", "options": "-sV; touch test-artifact"},
    ...
    if INJECTION_ARTIFACT.exists():
        print_fail(f"CRITICAL: Command injection artifact exists: {INJECTION_ARTIFACT}")
        INJECTION_ARTIFACT.unlink()
        passed = False
    else:
...
""")
    ...
    if not test_server_up():
        ...
        print(" sudo systemctl start hexstrike")
